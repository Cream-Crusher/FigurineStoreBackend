...
class Users(Base):
    ...
    owned_blogs = relationship(Blogs, back_populates='owner', foreign_keys=[Blogs.owner_id])
    author_posts = relationship(Posts, back_populates='author', foreign_keys=[Posts.author_id])
    comments = relationship(Comments, back_populates='user', foreign_keys=[Comments.author_id])
